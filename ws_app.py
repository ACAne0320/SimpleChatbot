...
def handle_message(message):
    ...
    for t in range(max_length):
        ...
        if id2word.get(predicted_id, '_UNK') == '_EOS':
            break
        # 未预测出来的词用_UNK替代
        word = id2word.get(predicted_id, '_UNK')
        result += word
        dec_input = tf.expand_dims([predicted_id], 0)
        emit('response', {'text': word})
    emit('response', {'text': "<end>"})


if __name__ == '__main__':
    socketio.run(app, host='127.0.0.1', port=8808, debug=True)
